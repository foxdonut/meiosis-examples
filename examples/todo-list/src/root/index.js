      ...
      <div>
        <Menu widths={2}>
          <Menu.Item
            ...
            onClick={() => root.update(actions.navigateTo("TodoListPage"))}
          >
            Todo List
          </Menu.Item>
          <Menu.Item
            active={root.state.pageId === "ProjectPage"}
            onClick={() => root.update(actions.navigateTo("ProjectPage"))}
          >
            Projects
          </Menu.Item>
        </Menu>
        ...
        <Modal open={root.state.error != null} size="small">
          ...
          <Modal.Actions>
            <Button onClick={() => root.update(actions.clearError())}>Ok</Button>
          </Modal.Actions>
        </Modal>
      </div>
    )
